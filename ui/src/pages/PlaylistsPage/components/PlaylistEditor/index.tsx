  ...
  const added = (
    <Box padding="10px" flexDirection="column" gap={10} width="100%">
      ...
      {!(isFetching || chainFetch.isFetching) && tracks.map((item) => {
        ...
        return (
          <Box key={item.fileItemHash} alignItems="center" justifyContent="space-between" width="100%">
            ...
            <Button onClick={() => handleDelete(item.fileItemHash)} variant="secondary" size="small">✕</Button>
          </Box>
        );
      })}
    </Box>
