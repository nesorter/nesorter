...
export const PlaylistBox = () => {
  ...
  useEffect(() => {
    api.scanner.getChain().then(setChain).catch(alert);
  }, []);
      ...
      <table>
        ...
        <tbody>
          {status.queue.items.map((track) => {
            ...
            const trackName = chainItem
              ? `${chainItem.fsItem?.metadata?.artist} - ${chainItem.fsItem?.metadata?.title}`
              : track.fileHash;
            ...
            return (
              <tr key={track.fileHash}>
                ...
                <StyledTd className='start'>
                  {format(addSeconds(startOfDay(new Date()), track.startAt), 'HH:mm:ss')}
                </StyledTd>

                <StyledTd className='end'>
                  {format(addSeconds(startOfDay(new Date()), track.endAt), 'HH:mm:ss')}
                </StyledTd>
              </tr>
            );
          })}
